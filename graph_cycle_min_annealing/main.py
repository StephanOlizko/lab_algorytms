...
def nearest_neighbor_hamiltonian_cycle(graph, canvas):
    canvas.delete('all')
...
def hamiltonian_cycle(graph, canvas, pos, entry1, entry2, entry3, mode):
    canvas.delete('all')
    ...
    T = float(entry1.get())
    T_min = float(entry2.get())
    alpha = float(entry3.get())
    ...
    if mode == "default":
        ...
        current_solution = list(nearest_neighbor_hamiltonian_cycle(graph, canvas).nodes())
...
button1 = tk.Button(frame1, text='Построить гамильтонов цикл', command=lambda: hamiltonian_cycle(G1, canvas2, pos, entry1, entry2, entry3, mode.get()))
...
text1 = tk.Label(frame1, text='начальная температура', anchor='sw', height=2)
...
text2 = tk.Label(frame1, text='конечная температура', anchor='sw', height=2)
text2.pack(fill='both')
entry2 = tk.Entry(frame1, width=10)
entry2.pack(fill='both')
entry2.insert(0, '1')

text3 = tk.Label(frame1, text='коэффициент охлаждения', anchor='sw', height=2)
text3.pack(fill='both')
entry3 = tk.Entry(frame1, width=10)
entry3.pack(fill='both')
entry3.insert(0, '0.1')

#Добавь свитч на фрейме frame1, который будет менять режим работы алгоритма (по умолчанию или ближайший сосед)
mode = tk.StringVar()
mode.set('default')
radiobutton1 = tk.Radiobutton(frame1, text='по умолчанию', variable=mode, value='default')
radiobutton1.pack(fill='both')
radiobutton2 = tk.Radiobutton(frame1, text='ближайший сосед', variable=mode, value='nearest')
